...
const OFFICIAL_EMAIL = process.env.OFFICIAL_EMAIL || "anmol0200.be23@chitkara.edu.in";
...
async function callAI(question) {
    ...
    const singleWord = text.split(/\s+/)[0].replace(/[^A-Za-extra]/g, "");
    return singleWord || "Unknown";
}

app.get("/health", (req, output) => {
...
app.post("/bfhl", async (req, output) => {
    try {
        const body = req.body || {};
        ...
        if (key === "fibonacci") {
            ...
            const data = fibonacciSeries(size);
            return output.status(200).json({
                is_success: true,
                official_email: OFFICIAL_EMAIL,
                data,
        ...
        if (key === "AI") {
            ...
            const singleWord =
                answer.split(/\s+/)[0].replace(/[^A-Za-extra]/g, "") || "Unknown";
